...
export PATH=$WORKSPACE/phablet_tools:~/bin:$PATH
...
else if [[ $REPO_BRANCH =~ "phablet-saucy" ]]; then
   JENKINS_BUILD_DIR=ubuntu
else
...
phablet-dev-bootstrap -v galaxysl $JENKINS_BUILD_DIR
...
export PATH="$PATH:/opt/local/bin/:$PWD/prebuilts/misc/$(uname|awk '{print tolower($0)}')-x86/ccache"
export CCACHE_DIR=~/.ubuntu_ccache
else
